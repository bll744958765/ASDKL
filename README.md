...
Official implementation accompanying the ASDKL manuscript. This repository contains the proposed model, the synthetic-data generator, matched baselines, deterministic split utilities, ablation and sensitivity runners, and plotting code.

Trained checkpoints, logs, result tables, result CSVs, and rendered manuscript figures are intentionally excluded. The only included image is this generator-only preview:
...
methods/      ASDKL and 13 baselines
...
The common baseline set is SimpleKriging, RegressionKriging, RFKriging, RandomForest, ExtraTrees, HistGBR, Coordinate-MLP, DeepKriging-adapted, 4N-adapted, KCN-adapted, PE-GNN-adapted, NNGP-Matern, and SparseGP-Nystrom.

```bash
python methods/baselines/_shared/run_baseline.py --datasets synthetic pm25 lucas_large california --seeds 1042 1052 1062 1072 1082 --data-root data --output-dir results/baselines
```

Methods marked `adapted` preserve the main mechanism of the cited family under the common data protocol; they are not claimed to reproduce every setting of an external repository.
